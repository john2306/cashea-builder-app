...
function rangeIncludesDate(range, date, excludeEnds = false, dateLib = defaultDateLib) {
  let { from, to } = range;
  const { differenceInCalendarDays: differenceInCalendarDays2, isSameDay } = dateLib;
  if (from && to) {
...
function dateMatchModifiers(date, matchers, dateLib = defaultDateLib) {
  ...
  const { isSameDay, differenceInCalendarDays: differenceInCalendarDays2, isAfter } = dateLib;
...
function getDates(displayMonths, maxDate, props, dateLib) {
  ...
  const { addDays, differenceInCalendarDays: differenceInCalendarDays2, differenceInCalendarMonths: differenceInCalendarMonths2, endOfBroadcastWeek, endOfISOWeek, endOfMonth, endOfWeek, isAfter, startOfBroadcastWeek, startOfISOWeek, startOfWeek } = dateLib;
...
function getInitialMonth(props, navStart, navEnd, dateLib) {
  ...
  const { differenceInCalendarMonths: differenceInCalendarMonths2, addMonths, startOfMonth } = dateLib;
...
function getMonths(displayMonths, dates, props, dateLib) {
  const { addDays, endOfBroadcastWeek, endOfISOWeek, endOfMonth, endOfWeek, getISOWeek: getISOWeek2, getWeek: getWeek2, startOfBroadcastWeek, startOfISOWeek, startOfWeek } = dateLib;
...
function getNextMonth(firstDisplayedMonth, calendarEndMonth, options, dateLib) {
  ...
  const { startOfMonth, addMonths, differenceInCalendarMonths: differenceInCalendarMonths2 } = dateLib;
...
function useCalendar(props, dateLib) {
  ...
  const { months, weeks, days, previousMonth, nextMonth } = (0, import_react29.useMemo)(() => {
    ...
    const dates = getDates(displayMonths, props.endMonth ? endOfMonth(props.endMonth) : void 0, {
...
function useMulti(props, dateLib) {
  ...
  const isSelected = (date) => {
    return (selected == null ? void 0 : selected.some((d) => isSameDay(d, date))) ?? false;
  };
  const { min, max } = props;
  const select = (triggerDate, modifiers, e) => {
    let newDates = [...selected ?? []];
    if (isSelected(triggerDate)) {
      if ((selected == null ? void 0 : selected.length) === min) {
        return;
      }
      if (required && (selected == null ? void 0 : selected.length) === 1) {
        return;
      }
      newDates = selected == null ? void 0 : selected.filter((d) => !isSameDay(d, triggerDate));
    } else {
      if ((selected == null ? void 0 : selected.length) === max) {
